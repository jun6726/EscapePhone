...
struct MainMenuView: View {
    ...
    var onBackToThemeSelection: (() -> Void)?
        ...
        ZStack(alignment: .topLeading) {
            ...
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    Text("THE LAST COMMIT").font(.system(.largeTitle, design: .monospaced, weight: .bold)).multilineTextAlignment(.center).foregroundStyle(AppTheme.textPrimary)
                    Text("사라진 개발자의 휴대폰").foregroundStyle(AppTheme.textSecondary)
                    ...
                    VStack(spacing: 12) {
                        ...
                        Button("설정") { app.navigate(.settings) }.buttonStyle(SecondaryButtonStyle())
                        Button("게임 소개") { showAbout = true }.buttonStyle(SecondaryButtonStyle())
                        if app.hasSavedGame { Button("게임 초기화") { confirmReset = true }.buttonStyle(GhostButtonStyle()).foregroundStyle(AppTheme.warning) }
                    }.padding(.top, 28)
                    Text("25–35분 · 익명 분석 선택 · 센서 또는 터치").font(.footnote).foregroundStyle(AppTheme.textSecondary)
                }.padding(28).padding(.top, 36).frame(maxWidth: 520)
            }
            BackButton { (onBackToThemeSelection ?? app.returnToThemeSelection)() }.padding(20)
        }
