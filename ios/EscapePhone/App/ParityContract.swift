...
struct EncryptedNoteView: View {
    ...
    var body: some View { PuzzleContainer("암호화 메모") {
        TextField("3자리 커밋 번호", text: $code).keyboardType(.numberPad).textFieldStyle(.roundedBorder).onChange(of: code) { _, value in code = String(value.filter(\.isNumber).prefix(3)) }
        if !engine.isUnlocked {
            Text("손전등에서 발견한 조작 커밋 번호로 메모를 여세요.").foregroundStyle(.secondary)
            Button("메모 열기") { error = !engine.unlock(code: code); if error { app.recordWrongAttempt("encrypted_note", reason: "unlockCodeIncorrect") } }.buttonStyle(PrimaryButtonStyle()).disabled(code.count != 3)
        } else {
            ...
            HStack { Button("마지막 단어 제거") { engine.removeNoteWord() }; Button("문장 확인") { if engine.submitEncryptedNote() { app.completeEncryptedNotePuzzle() } else { error = true; app.recordWrongAttempt("encrypted_note", reason: engine.selectedWords.count < 3 ? "noteWordsIncomplete" : "noteWordOrderIncorrect") } } }
            if app.gameProgress.encryptedNoteSolved { Button("음성 기록 열기") { app.navigate(.audioRecordPuzzle) }.buttonStyle(PrimaryButtonStyle()) }
        }
        if error { Text("입력과 단어 순서를 확인하세요.").foregroundStyle(.red) }
        HintPair("encrypted_note", ["메모의 형식은 [대상] [동작] [진행 상태]로, 빈칸은 총 세 칸입니다.", "대상=기록, 동작=수집, 진행 상태=중입니다. 따라서 ‘기록 수집 중’입니다."])
    } }
}
